...
var fnSegmentReady_triggered = false;
//FS - Full Story
...
function analyticsReady() {
    ...
    if (typeof segmentReady == 'function' && !fnSegmentReady_triggered) {
    ...
    !function () {
        ...
        var analytics = window.analytics = window.analytics || []; if (!analytics.initialize) if (analytics.invoked) window.console && console.error && console.error("Segment snippet included twice."); else {
            ...
            if (typeof segmentReady == 'function') {
                try { segmentReady(); fnSegmentReady_triggered = true; } catch (e) { console.log(e); }
            }
...
function getGAId() {
    var gaVal = null;
    try {
        var gaId = jsUtility.get_Cookie('_ga');
        if (gaId && gaId.trim().length > 0) {
            var gaItems = gaId.split('.');
            gaVal = gaItems[gaItems.length - 2] + "." + gaItems[gaItems.length - 1];
        }
    }
    catch (e) {
        console.log(e);
...
function TrackEvents(eventname, eventpropval, userid, islogin, skipTraitsToIterable, contextprops, callback) {
    ...
    //Add Portal if missing in properties
    if (segment_portal_name)
        propertiesToBeSent["Portal"] = segment_portal_name;

    switch (eventname) {
        case "identify":
            {
                FireSegmentIOIdentify(userid, eventproperties, skipTraitsToIterable, contextprops);
                break;
            }
        case "page":
            {
                ...
                var gaValue = getGAId();
                if (gaValue && propertiesToBeSent && gaValue.length > 0)
                    propertiesToBeSent["GA_Id"] = gaValue;
                FireSegmentIOPage("", propertiesToBeSent);
                break;
            }
        default:
            {
                FireSegmentIOTrack(eventname, propertiesToBeSent, callback);
            }
    }
    SetMixpanelPropsCookie();
}
...
function FireSegmentIOIdentify(userid, traits, skipTraitsToIterable, contextprops) {
    traits = traits || null;
    ...
    TrackSegmentContext(userid, contextprops);
    //clear traits
...
function FireSegmentIOTrack(eventname, properties, callback) {
    try {
        properties = properties || null;
        if (properties != null) {
...
//Set Context properties like locale if set. As of now only locale supported
function TrackSegmentContext(userid, contextprops) {
    if (userid && typeof contextprops != 'undefined' && typeof contextprops["locale"] != 'undefined') {
        if (typeof (Storage) !== "undefined") {
            localStorage.setItem("segment_context_locale", contextprops["locale"]);
        }
    }
    var segment_context_locale = localStorage.getItem("segment_context_locale");
    if (userid && typeof segment_context_locale != 'undefined' && segment_context_locale != null) {
        analytics.identify(userid, {}, {
            integrations: {
                "All": false,
                'Iterable': true
            },
            "context": { "locale": segment_context_locale }
        });
    }
}
/************Added Later *********/
